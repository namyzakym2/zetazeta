...
(async()=>{if(!process.env.CONTROL_BOT_TOKEN)throw new Error('CONTROL_BOT_TOKEN missing');if(!process.env.OWNER_ID)throw new Error('OWNER_ID missing');await db.connect();await client.login(process.env.CONTROL_BOT_TOKEN)})().catch(e=>{console.error('Control bot failed:',e);process.exit(1)});
